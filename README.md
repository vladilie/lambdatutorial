...
```bash
aws cloudformation delete-stack --stack-name lamdbatutorial
```
...
For http :
docker run -d \
    -v <pwd>/resources/http/files:/web \
    -v <pwd>/cert:/cert \
    -p 8300:8080 \
    -e TLS_CERT=cert/localhost.crt \
...
docker run \
    -v <pwd>/resources/ftp/files:/home/foo/upload \
    -p 2222:22 -d atmoz/sftp \
    foo:pass:1001

Due to the use of self signed certificates one needs to configure nodejs to allow unsecure connections - this should be 
used only for local development testing debugging.

sam local invoke HttpsScheduledEventLogger --event events/event-cloudwatch-event.json --env-vars config/env.json